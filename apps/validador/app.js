...
const SyncManager = {
    ...
    updateUI(synced) {
        ...
        if (el) {
            ...
            if (synced) {
                ...
                el.textContent = `⚠️ ${PENDING_SYNC.length} pendientes`;
            }
        }
        this.updateBadge();
    },
    ...
    updateBadge() {
        ...
        if (badge) {
            badge.innerHTML = PENDING_SYNC.length > 0
                ? `<span class="pending-sync-badge">${PENDING_SYNC.length}</span>`
                : '';
        }
    },

    showPanel() {
        showNotification(`📊 ${PENDING_SYNC.length} elementos pendientes de sincronizar`, 'info');
    }
};
...
async function loadFromStorage() {
    const savedState = localStorage.getItem('wms_state');
    if (savedState) STATE = JSON.parse(savedState);

    const savedHistory = localStorage.getItem('wms_history');
    if (savedHistory) HISTORY = new Map(JSON.parse(savedHistory));

    const savedBD = localStorage.getItem('wms_bd_codes');
    if (savedBD) {
        const data = JSON.parse(savedBD);
        BD_CODES = new Set(data.codes || []);
        OBC_MAP = new Map((data.obcMap || []).map(([k, v]) => [k, new Set(v)]));
        OBC_TOTALS = new Map(data.totals || []);
        OBC_INFO = new Map(data.info || []);
        LAST_BD_UPDATE = data.lastUpdate;
    }
}

async function saveState() {
    localStorage.setItem('wms_state', JSON.stringify(STATE));
}

async function saveBD() {
    localStorage.setItem('wms_bd_codes', JSON.stringify({
...
async function saveHistory() {
    localStorage.setItem('wms_history', JSON.stringify(Array.from(HISTORY.entries())));
}

async function loadPendingSync() {
    return SyncManager.load();
}

async function loadPrerecData() {
    const saved = localStorage.getItem('wms_prerec_data');
    if (saved) PREREC_DATA = new Map(JSON.parse(saved));
}

async function savePrerecData() {
    localStorage.setItem('wms_prerec_data', JSON.stringify(Array.from(PREREC_DATA.entries())));
}
...
function exportData() {
    const rows = ['Fecha,Hora,Usuario,Orden,Código,Ubicación,Nota'];
    const allOrders = { ...STATE.tabs, ...STATE.closedTabs };
...
function showUserStats() {
    ...
    statsContent.innerHTML = sortedUsers.map(([user, count]) => `
        <div class="user-stat-item">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <span class="user-stat-name">${user}</span>
                <span class="user-stat-count">${count}</span>
            </div>
            <div class="user-stat-bar" style="width: ${(count / maxCount) * 100}%"></div>
        </div>
    `).join('') || '<p style="text-align: center; color: #999;">Sin validaciones</p>';
...
function syncPendingData(isManual = false) { return SyncManager.sync(isManual); }
function savePendingSync() { return SyncManager.save(); }
function updatePendingBadge() { SyncManager.updateBadge(); }
function updateSyncStatus(synced) { SyncManager.updateUI(synced); }
function forceSync() { SyncManager.sync(true); }
